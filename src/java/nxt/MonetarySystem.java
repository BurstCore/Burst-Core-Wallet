...
public abstract class MonetarySystem extends ChildTransactionType {
    ...
    public static final TransactionType RESERVE_INCREASE = new MonetarySystem() {
        ...
        boolean applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            Attachment.MonetarySystemReserveIncrease attachment = (Attachment.MonetarySystemReserveIncrease) transaction.getAttachment();
            Currency currency = Currency.getCurrency(attachment.getCurrencyId());
            if (senderAccount.getUnconfirmedBalance((ChildChain)transaction.getChain()) >= Math.multiplyExact(currency.getReserveSupply(), attachment.getAmountPerUnitNQT())) {
        ...
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.MonetarySystemReserveIncrease attachment = (Attachment.MonetarySystemReserveIncrease) transaction.getAttachment();
            CurrencyFounderHome.forChain((ChildChain)transaction.getChain()).increaseReserve(getLedgerEvent(), transaction.getId(), senderAccount, attachment.getCurrencyId(),
    ...
    public static final TransactionType RESERVE_CLAIM = new MonetarySystem() {
        ...
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.MonetarySystemReserveClaim attachment = (Attachment.MonetarySystemReserveClaim) transaction.getAttachment();
            Currency.claimReserve((ChildChain)transaction.getChain(), getLedgerEvent(), transaction.getId(), senderAccount, attachment.getCurrencyId(),
                    attachment.getUnits());
        }
    ...
    public static final TransactionType PUBLISH_EXCHANGE_OFFER = new MonetarySystem() {
        ...
        boolean applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            Attachment.MonetarySystemPublishExchangeOffer attachment = (Attachment.MonetarySystemPublishExchangeOffer) transaction.getAttachment();
            if (senderAccount.getUnconfirmedBalance((ChildChain)transaction.getChain()) >= Math.multiplyExact(attachment.getInitialBuySupply(), attachment.getBuyRateNQT())
        ...
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.MonetarySystemPublishExchangeOffer attachment = (Attachment.MonetarySystemPublishExchangeOffer) transaction.getAttachment();
            ExchangeOfferHome.forChain((ChildChain)transaction.getChain()).publishOffer(transaction, attachment);
        }
    ...
    public static final TransactionType EXCHANGE_BUY = new MonetarySystemExchange() {
        ...
        boolean applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            ...
            if (senderAccount.getUnconfirmedBalance((ChildChain)transaction.getChain()) >= Math.multiplyExact(attachment.getUnits(), attachment.getRateNQT())) {
                senderAccount.addToUnconfirmedBalance((ChildChain)transaction.getChain(), getLedgerEvent(), transaction.getId(),
        ...
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.MonetarySystemExchangeBuy attachment = (Attachment.MonetarySystemExchangeBuy) transaction.getAttachment();
            ExchangeRequestHome.forChain((ChildChain)transaction.getChain()).addExchangeRequest(transaction, attachment);
            ExchangeOfferHome.forChain((ChildChain)transaction.getChain()).exchangeNXTForCurrency(transaction, senderAccount, attachment.getCurrencyId(), attachment.getRateNQT(), attachment.getUnits());
        }
    ...
    public static final TransactionType EXCHANGE_SELL = new MonetarySystemExchange() {
        ...
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.MonetarySystemExchangeSell attachment = (Attachment.MonetarySystemExchangeSell) transaction.getAttachment();
            ExchangeRequestHome.forChain((ChildChain)transaction.getChain()).addExchangeRequest(transaction, attachment);
            ExchangeOfferHome.forChain((ChildChain)transaction.getChain()).exchangeCurrencyForNXT(transaction, senderAccount, attachment.getCurrencyId(), attachment.getRateNQT(), attachment.getUnits());
        }
